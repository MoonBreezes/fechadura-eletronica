...
''' Insert user in db
    ...
    @param role: string among ['admin', 'lock', 'lock_user']
...
def insert_user(db, role, atributes={}):
    ...
    if role == 'admin' or 'username' == '0':
        ...
        raise ValueError("'role' must be one of ['admin', 'lock', 'lock_user']")
...
def check_roles(room, user):
    ...
    if room.role is 'lock' and user.role in ['lock_user']:
